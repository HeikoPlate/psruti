// StartBeatFolderDlg.cpp: Implementierungsdatei
//
...
StartBeatFolderDlg::StartBeatFolderDlg(CWnd* pParent /*=NULL*/)
	: CDialog(StartBeatFolderDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(StartBeatFolderDlg)
		// HINWEIS: Der Klassen-Assistent fügt hier Elementinitialisierung ein
	//}}AFX_DATA_INIT

	drum = NO_DRUMS+FIRST_DRUM; // initialized for delete countin
	velocity = 0;
	start_beat_delete = true;
}

const char* StartBeatFolderDlg::drums[NO_DRUMS+1] =
{
	"C#1: Surdo Mute",		// 13
	"D1:  Surdo Open",		// 14
	"Eb1: Hi Q",			// 15
...
void StartBeatFolderDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(StartBeatFolderDlg)
	...
	DDX_Control(pDX, IDC_INSERT_COUNT_IN, m_insert_count_in);
	DDX_Control(pDX, IDC_DELETE_COUNT_IN, m_delete_count_in);
	DDX_Control(pDX, IDC_CHECK_ALL_BEATS, m_check_all_beats);
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(StartBeatFolderDlg, CDialog)
	//{{AFX_MSG_MAP(StartBeatFolderDlg)
	ON_EN_CHANGE(IDC_START_BEAT_VEL, OnChangeStartBeatVel)
	ON_BN_CLICKED(IDC_CHECK_ALL_BEATS, OnCheckAllBeats)
	ON_BN_CLICKED(IDC_DELETE_COUNT_IN, OnDeleteCountIn)
	ON_BN_CLICKED(IDC_INSERT_COUNT_IN, OnInsertCountIn)
	ON_WM_CANCELMODE()
...
/////////////////////////////////////////////////////////////////////////////
// Behandlungsroutinen für Nachrichten StartBeatFolderDlg 

void StartBeatFolderDlg::OnOK() 
{
...
void StartBeatFolderDlg::OnCancel() 
{
	EndDialog(-1);
}


BOOL StartBeatFolderDlg::OnInitDialog() 
{
...
void StartBeatFolderDlg::OnChangeStartBeatVel() 
{
...
bool StartBeatFolderDlg::get_start_beat_delete()
{
	return start_beat_delete;
}

int StartBeatFolderDlg::get_velocity()
{
	return velocity;
}

int StartBeatFolderDlg::get_drum()
{
	return drum;
}

void StartBeatFolderDlg::set_velocity(int vel)
{
	velocity = vel;
}

void StartBeatFolderDlg::set_drum(int note)
{
	drum = note;
}

void StartBeatFolderDlg::OnCheckAllBeats() 
{
	on_all_beats = m_check_all_beats.GetCheck()==1;	
}


void StartBeatFolderDlg::OnCheckOneMeasure() 
{
	only_one_measure = m_check_one_measure.GetCheck()==1;
}

void StartBeatFolderDlg::OnDeleteCountIn() 
{
	if (m_delete_count_in.GetCheck())
	{
		start_beat_delete = true;	
		m_drum.EnableWindow(false);
		m_start_beat_vel.EnableWindow(false);
		m_check_all_beats.EnableWindow(false);
		m_insert_count_in.SetCheck(0);
		SetDlgItemText(IDC_START_BEAT_INS_DEL,
			GlobalUtilities::get_resource_string(IDS_COUNT_IN_DELETE));
	}
	else
	{
		start_beat_delete = false;	
		m_start_beat_vel.EnableWindow(true);
		m_check_all_beats.EnableWindow(true);
		m_drum.EnableWindow(true);
		m_delete_count_in.SetCheck(0);
		SetDlgItemText(IDC_START_BEAT_INS_DEL,
			GlobalUtilities::get_resource_string(IDS_COUNT_IN_INSERT));
	}
}

void StartBeatFolderDlg::OnInsertCountIn() 
{
	if (!m_insert_count_in.GetCheck())
	{
		start_beat_delete = true;	
		m_drum.EnableWindow(false);
		m_start_beat_vel.EnableWindow(false);
